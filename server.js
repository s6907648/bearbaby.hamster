...
async function initDB() {
 ...
 await client.query(`INSERT INTO shop (id, shop_name, logo_emoji, intro, bank_name, account, note) VALUES (1, '倉鼠小窩', '🐹', '用心照顧每一隻小生命', '', '', '') ON CONFLICT (id) DO NOTHING`);
 }
...
app.use((req, res, next) => { res.set("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0"); next(); });
...
app.get("/api/products", async (req, res) => {
 try { const r = await pool.query("SELECT * FROM products ORDER BY sort_order, created_at"); json(res, r.rows); } catch(e) { err(res, 500, e.message); }
});

app.post("/api/products", async (req, res) => {
 try {
 const { name, category = 'goods', description = '', price = 0, status = 0 } = req.body;
 const r = await pool.query(`INSERT INTO products (name, category, description, price, status) VALUES ($1, $2, $3, $4, $5) RETURNING *`, [name, category, description, price, status]);
 const p = r.rows[0];
...
app.put("/api/products/:id", async (req, res) => {
 ...
 const r = await pool.query(`UPDATE products SET name=$1, category=$2, description=$3, price=$4, status=$5, sort_order=$6, updated_at=NOW() WHERE id=$7 RETURNING *`, [name, category, description, price, status, sort_order, req.params.id]);
